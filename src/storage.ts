  ...
  try {
    ...
    if (data.quotaTracking) {
      for (const [email, tracking] of Object.entries(data.quotaTracking)) {
        ...
        delete (tracking as any).calibration;
        delete (tracking as any).calibrations;
      }
    }
 ...
 */
export function getQuotaCacheFilePath(): string {
 ...
 */
function modelsToGroups(models: ServerQuotaModel[]): ServerQuotaGroup[] {
  const groupsMap: Record<string, { models: string[]; pct: number | null; reset: string | null }> = {
 ...
 */
export async function loadServerQuotaCache(): Promise<ServerQuotaCache | null> {
  try {
    if (!existsSync(QUOTA_CACHE_FILE)) {
      return null;
    }
    const content = await readFile(QUOTA_CACHE_FILE, "utf-8");
    const data = JSON.parse(content) as ServerQuotaCache;
 ...
 */
export async function saveServerQuotaCache(cache: ServerQuotaCache): Promise<void> {
  try {
    // Ensure directory exists
