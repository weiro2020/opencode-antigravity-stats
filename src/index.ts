...
export const plugin: Plugin = async ({ client }) => {
  ...
  return {
    ...
    tool: {
      "antigravity-stats": {
        description:
          "Show Antigravity usage statistics including tokens, rate-limits, and errors",
        parameters: z.object({
          view: z
            ...
            .default("all")
            .describe(
              "View type: session (current), daily (last 7 days), errors (errors & rate-limits), all (everything)"
            ),
        }),
