...
import type { StatsData, DailyStats, ModelStats } from "./types.js";
 ...
 */
function formatSession(stats: StatsData): string {
  ...
  if (models.length === 0) {
    ...
    for (const [model, modelStats] of models) {
      ...
      lines.push(`  ${model}`);
      lines.push(
        `    ${modelStats.requests} req | ${tokensIn} in | ${tokensOut} out` +
  ...
  lines.push("");
  lines.push(
    `  Totales: ${session.totals.requests} req | ` +
 ...
 */
function formatDaily(stats: StatsData): string {
  const lines: string[] = [];
  ...
  if (totalRequests === 0) {
    ...
    lines.push("");
    lines.push(
      `  Total:   ${totalRequests} req | ${formatNumber(totalTokensIn)} in | ` +
 ...
 */
function formatErrors(stats: StatsData, accountsStats?: AccountStatsInfo[]): string {
  const lines: string[] = [];
  ...
  const accountStats: Record<string, { requests: number; rateLimits: number }> = {};
...
export function formatStats(
  stats: StatsData,
  view: "session" | "daily" | "errors" | "all" = "all",
  accountsStats?: AccountStatsInfo[]
): string {
  const header = [
    "═".repeat(60),
    "                 ANTIGRAVITY STATS",
